...
app.get("/pokemon/:id/moves", async (req: Request, res: Response) => {
  try {
    ...
    if (result.rows.length === 0) {
      res
        .status(404)
        .json({ status: "failure", error: "No moves found for this Pokémon." });
      return;
    }
    ...
    res
      .status(500)
      .json({ status: "failure", error: "Failed to fetch Pokémon moves." });
  }
});
...
app.get("/pokemon", async (req: Request, res: Response) => {
  try {
    ...
    const searchTerm = `%${search}%`; // wildcard for partial search matching

    // query to get paginated data
    const dataQuery = `SELECT p.*, (
    SELECT json_agg(json_build_object(
    'to_id', e.to_pokemon_id,
    'name', p_to.name,
    'requirement', e.requirement
    ...
    const countQuery = `SELECT COUNT(*) FROM pokemon WHERE name ILIKE $1 OR id::text ILIKE $1;`;
